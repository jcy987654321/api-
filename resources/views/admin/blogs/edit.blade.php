...
<div class="card admin-card">
    <div class="card-header">
        <h5 class="card-title mb-0">Edit Post: {{ Str::limit($blog->title, 30) }}</h5>
    </div>
    <div class="card-body">
        @if($errors->any())
            ...
            <div class="row">
                <div class="col-lg-8">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="excerpt" id="excerpt" class="form-control @error('excerpt') is-invalid @enderror" rows="3" maxlength="500">{{ old('excerpt', $blog->excerpt) }}</textarea>
                    ...
                    <div class="card mb-3">
                        ...
                        <div class="card-body">
                            <div class="mb-3">
                                ...
                                <input type="url" name="cover_image" id="cover_image" class="form-control @error('cover_image') is-invalid @enderror" value="{{ old('cover_image', $blog->cover_image) }}" placeholder="https://...">
                                @error('cover_image')
                                    <div class="invalid-feedback d-block">{{ $message }}</div>
                                @enderror
                            </div>
                            <div class="preview-container">
                                <img id="coverPreview" src="{{ $blog->cover_image }}" alt="Preview" class="img-thumbnail {{ !$blog->cover_image ? 'd-none' : '' }}" style="max-width: 100%;">
                            </div>
                        </div>
                    </div>
                                ...
                                <div class="tags-selection" style="max-height: 200px; overflow-y: auto;">
                                    @foreach($tags as $tag)
                                        <div class="form-check">
                                            <input type="checkbox" name="tags[]" id="tag_{{ $tag->id }}" value="{{ $tag->id }}" class="form-check-input" {{ in_array($tag->id, old('tags', $blog->tags->pluck('id')->toArray())) ? 'checked' : '' }}>
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            <ul class="list-unstyled mb-0 small">
                                <li><strong>Created:</strong> {{ $blog->created_at->format('M d, Y H:i') }}</li>
                                <li><strong>Updated:</strong> {{ $blog->updated_at->format('M d, Y H:i') }}</li>
                                <li><strong>Views:</strong> {{ number_format($blog->views) }}</li>
                            </ul>
                        </div>
                    </div>
                </div>
